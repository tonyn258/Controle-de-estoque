...
*/
 ...
 class Itens extends Connect
 {
  ...
  public function listItens($skuProduto)
  {
      $query = "SELECT skuProduto, NomeProduto, SUM(QuantItens) AS QuantItens, SUM(QuantItensVend) AS QuantItensVend
                FROM compras
                WHERE skuProduto = '$skuProduto'
                GROUP BY skuProduto, NomeProduto";
      $result = mysqli_query($this->SQL, $query) or die(mysqli_error($this->SQL));
  
      $items = array();
      
      while ($rowlist = mysqli_fetch_array($result)) {
          $q = $rowlist['QuantItens'];
          $v = $rowlist['QuantItensVend'];
          $e = $q - $v;
...
{
    $query = "SELECT DISTINCT skuProduto
              FROM compras
              WHERE `public` = '$value'";
    $result = mysqli_query($this->SQL, $query) or die(mysqli_error($this->SQL));
    ...
    while ($row = mysqli_fetch_assoc($result)) {
        ...
        $item = $this->listItens($skuProduto);
        $items[] = $item[0]; // Adicione [0] para acessar apenas o primeiro item do array
    }

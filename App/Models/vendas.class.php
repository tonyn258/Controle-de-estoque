...
{
    public function intesVendidos($idItem, $quant, $NomeCliente, $cpfCliente, $FoneCliente, $idUsuario, $DataVenda, $CodRastreioV, $Vd_Tax)
    {
        // Verificar se o item de compra existe
        $this->query = "SELECT * FROM `anuncio` WHERE `IdCompra` = '$idItem'";
        ...
        if ($this->result) {
            ...
            if ($row = mysqli_fetch_array($this->result)) {
                ...
                if ($q >= $quantotal) {
                    ...
                    VALUES (NULL,'$NomeCliente','$FoneCliente','$cpfCliente',1,'$idUsuario')";
                        if (mysqli_query($this->SQL, $this->NovoClient) or die(mysqli_error($this->SQL))) {
                            $idCliente = mysqli_insert_id($this->SQL);
                        }
                    }
                    // Registrar a venda
                    $this->query = "INSERT INTO `vendas`(`Itensquant`,`anuncio_id`, `valor`, `IdItem`,`cliente_idCliente`,`anuncio_data`, `DataVenda`, `CodRastreioV`,`TxMl`,`TxFret`,`Vd_Tax`) 
                                                VALUES ('$quant','$Compra_id','$valor','$idItem','$idCliente','$compra_idData','$DataVenda','$CodRastreioV','$TxMl','$TxFret','$Vd_Tax')";
                    if ($this->result = mysqli_query($this->SQL, $this->query) or die(mysqli_error($this->SQL))) {
                        // Atualizar a quantidade de itens vendidos na tabela de compras
                        $this->query = "UPDATE `anuncio` SET `QuantItensVend` = '$quantotal' WHERE `IdCompra`= '$idItem '";
                        if ($this->result = mysqli_query($this->SQL, $this->query) or die(mysqli_error($this->SQL))) {
